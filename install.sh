...
collect_answers() {
  ...
  printf "[SG-Gateway] Все вопросы задаются до основной установки. Домен не требуется.\n\n"

  PUBLIC_ADDRESS="$(detect_public_ip || true)"
  if ! valid_public_ipv4 "$PUBLIC_ADDRESS"; then
    read_tty "Публичный IPv4 сервера" PUBLIC_ADDRESS ""
    valid_public_ipv4 "$PUBLIC_ADDRESS" || { echo "Не удалось определить корректный публичный IPv4." >&2; return 1; }
  fi
  COUNTRY_CODE="$(detect_country_code "$PUBLIC_ADDRESS")"
  printf "[SG-Gateway] Определён публичный IP: %s\n" "$PUBLIC_ADDRESS"
  printf "[SG-Gateway] Страна сервера: %s\n" "${COUNTRY_CODE^^}"

  local default_server_name="sg-gateway"
  [[ "$COUNTRY_CODE" != "unknown" ]] && default_server_name="sg-gateway-${COUNTRY_CODE}"
  read_tty "Имя сервера и hostname SSH" SERVER_NAME "$default_server_name"
  SERVER_NAME="$(normalize_hostname "$SERVER_NAME")"
  valid_hostname "$SERVER_NAME" || { echo "Недопустимое имя сервера." >&2; return 1; }

  read_tty "Публичный HTTP-порт панели" PANEL_PORT "$DEFAULT_PANEL_PORT"
  valid_port "$PANEL_PORT" || { echo "Недопустимый порт панели." >&2; return 1; }
  read_tty "Reality target" REALITY_TARGET "$DEFAULT_REALITY_TARGET"
  read_tty "Reality SNI" REALITY_SNI "$DEFAULT_REALITY_SNI"
  read_tty "Порт VLESS Reality TCP" XRAY_PORT "$DEFAULT_XRAY_PORT"
  valid_port "$XRAY_PORT" || { echo "Недопустимый порт Xray." >&2; return 1; }
  AWG_PORT="$DEFAULT_AWG_PORT"
  printf "[SG-Gateway] AmneziaWG: фиксированный UDP-порт %s\n" "$AWG_PORT"
  if [[ "$PANEL_PORT" == "$XRAY_PORT" ]]; then
