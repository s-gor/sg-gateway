...
def test_client_exports_include_generated_values(tmp_path, monkeypatch):
    ...
    assert "PLACEHOLDER_CLIENT_PRIVATE_KEY" not in awg.body
    assert "vless://" in xray.body
    assert "PLACEHOLDER_UUID" not in xray.body
