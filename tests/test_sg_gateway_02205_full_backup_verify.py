...
def test_full_backup_verify_accepts_restore_compatible_archive(monkeypatch, tmp_path):
    ...
    assert payload["source_version"] == "0.1.0-022.05"
    ...
    assert not archive.exists(), "verification upload must be deleted after success"
    assert not (data_dir / "sg-gateway.sqlite").exists(), "verification must not touch the live database"
...
def test_full_backup_verify_rejects_broken_archive_and_cleans_upload(monkeypatch, tmp_path):
    ...
    assert not archive.exists(), "broken verification upload must also be deleted"
