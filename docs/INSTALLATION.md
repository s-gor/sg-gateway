...
Используется та же команда. При обновлении существующие пароль, параметры и клиенты сохраняются. Перед изменением создаётся резервная копия управляемых файлов и состояния служб.
