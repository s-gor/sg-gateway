...
| Mihomo Core | Mieru, AnyTLS и TUIC v5 |
...
Один Mihomo Core обслуживает три независимых listener:

| Listener | Транспорт | Включение |
|---|---|---|
| Mieru | TCP или UDP | Отдельная галочка |
| AnyTLS | TCP | Отдельная галочка |
| TUIC v5 | UDP | Отдельная галочка |

Раскрытие секции «Дополнительно» не меняет состояние listener. Протокол включается только соответствующим переключателем и последующим применением. Выключенный listener удаляется из live-конфигурации Mihomo, перестаёт слушать порт и не экспортируется в ссылки, QR-коды и subscription. Реквизиты сохраняются для повторного включения.

TLS-готовность Mihomo определяется через публичное состояние:

```text
/var/lib/sg-gateway/security/tls-state.json
```

Непривилегированный процесс не проверяет закрытые файлы `/etc/letsencrypt/live/...` напрямую.
