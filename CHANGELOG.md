# 0.1.0-021.9 — персональные подписки и полный экспорт Mieru

- Кнопка подписки устройства копирует публичный URL `/sub/<token>`, а не внутреннее Base64-тело.
- Исправлены реальные переносы строк в декодированной подписке и имя профиля без расширения `.txt`.
- Общая SUB содержит только совместимые URI-профили; Mieru не смешивается с общей Base64-подпиской.
- Сохранена обычная ссылка `mierus://` и её отдельный QR для SG Client.
- Добавлен отдельный UTF-8 JSON Mieru с реальными адресом, портом и индивидуальными username/password устройства.
- Для Mieru JSON добавлены отдельные копирование, скачивание и QR.
- Дополнительный экспорт Mihomo YAML сохранён.
- Добавлена принятая мобильная версия меню: полноширинная навигация до 760 px, нормальные подписи и иконки, карточка сервера и настоящее сворачивание без узкой боковой рейки; desktop не затронут.
- В Routing технические Direct/WARP/Block получили понятные пользовательские подписи без изменения внутренних Xray-тегов.
- Добавлено редактирование клиентов и дополнительных устройств с изменением имени, срока и набора протоколов без перевыпуска неизменённых UUID, паролей, токенов и ссылок; при ошибке runtime выполняется откат.
