...
> **Временная восстановительная линия 021:** Mihomo и sing-box исключены из установки. Mieru, AnyTLS и TUIC временно не запускаются, пока проверяется стабильная установка Xray + AmneziaWG.
...
Один управляемый Mihomo Core предоставляет независимые listener:

- **Mieru**;
- **AnyTLS**;
- **TUIC v5**.

Каждый listener включается отдельно. После применения выключенный профиль удаляется из live-конфигурации, перестаёт слушать порт и исчезает из клиентских ссылок, QR-кодов и subscription. Раздел «Дополнительно» только показывает расширенные параметры и сам по себе ничего не включает.
