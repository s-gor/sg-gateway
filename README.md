...
![Версия](https://img.shields.io/badge/version-0.1.0--021.10-3b82f6)
...
### Принятые исправления после чистой установки на EC2

- Все карточки устройств открываются свёрнутыми и раскрываются независимо.
- У раскрытой карточки удалены старая белая внутренняя подложка и лишняя разделительная линия.
- В Recovery рядом с каждой резервной копией добавлена кнопка **«Восстановить»**.
- Кнопки восстановления в Recovery и Maintenance используют одну транзакцию с проверкой Xray и автоматическим rollback.
- Полная чистая установка проверена с HTTP 80, SNI-разделением TCP 443, Reality на внутреннем `127.0.0.1:7443`, HTTPS-заглушкой и панелью на 63443.
